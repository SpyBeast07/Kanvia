...
<div style="min-height: 100vh; display: flex; flex-direction: column; background: #0b1219;">
	...
	<header style="height: 120px; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; padding: 0 4rem; width: 100%;">
		...
		<div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1rem;">
			...
			<div style="background: #1e293b; color: #94a3b8; font-size: 0.7rem; padding: 0.2rem 0.5rem; border-radius: 0.25rem; font-weight: 900; margin-left: 0.25rem;">J</div>
	...
	{#if showNav}
		...
		>
			<div 
				...
				tabindex="-1"
			>
				<div style="color: var(--text-secondary); font-size: 0.75rem; font-weight: 700; text-transform: uppercase; margin-bottom: 1rem; padding: 0 0.5rem;">Navigation</div>
				{#each menuItems as item}
					<a 
						href={item.path} 
						onclick={() => showNav = false}
						style="display: flex; align-items: center; gap: 1rem; padding: 1rem; border-radius: 0.5rem; background: {page.url.pathname === item.path ? '#1e293b' : 'transparent'};"
